...
import type { TransformSetup } from "engine/shared/component/OldTransformService";
import type { RunningTransform } from "engine/shared/component/Transform";
...
type TweenableProperties<T> = ExtractKeys<Required<T>, Easable>;
type TransformProps = {
...
export class OldTransformBuilder<T extends object> {
	readonly instance;
	...
	build(): OldTransformRunner {
		return new OldTransformRunner(this.buildSequence());
	}
	...
	parallel(...funcs: ((transform: OldTransformBuilder<T>) => void)[]): this {
		const seq = funcs.map((func) => {
			const transform = new OldTransformBuilder<T>(this.instance);
			func(transform);
	...
	repeat(amount: number, func: (transform: OldTransformBuilder<T>) => void): this {
		const transform = new OldTransformBuilder<T>(this.instance);
	...
	other<T extends object>(instance: T, setup: TransformSetup<T>) {
		const transform = new OldTransformBuilder(instance);
	...
	setText<TKey extends ExtractKeys<T, string>>(
		this: OldTransformBuilder<T>,
		text: T[TKey],
		property: TKey,
		params?: TransformProps,
	): OldTransformBuilder<T> {
		return this.push(
			new TextTransform(
				this.instance,
				property,
	...
	/** Move the `GuiObject` */
	move(this: OldTransformBuilder<GuiObject>, position: UDim2, params?: TransformProps): OldTransformBuilder<T> {
		return this.transform("Position", position, params) as unknown as OldTransformBuilder<T>;
	}
	/** Move the `GuiObject` by X */
	moveX(this: OldTransformBuilder<GuiObject>, position: UDim, params?: TransformProps): OldTransformBuilder<T> {
		return this.transform(
			"Position",
			() => new UDim2(position, this.instance.Position.Y),
			params,
		) as unknown as OldTransformBuilder<T>;
	}
	/** Move the `GuiObject` by Y */
	moveY(this: OldTransformBuilder<GuiObject>, position: UDim, params?: TransformProps): OldTransformBuilder<T> {
		return this.transform(
			"Position",
	...
	resize(this: OldTransformBuilder<GuiObject>, size: UDim2, params?: TransformProps): OldTransformBuilder<T> {
		return this.transform("Size", size, params) as unknown as OldTransformBuilder<T>;
	}

	/** Relatively move the `GuiObject` */
	moveRelative(this: OldTransformBuilder<GuiObject>, offset: UDim2, params?: TransformProps) {
		return this.transform(
			"Position",
	...
	resizeRelative(
		this: OldTransformBuilder<GuiObject>,
		offset: UDim2,
		params?: TransformProps,
	): OldTransformBuilder<T> {
		return this.transform(
			"Size",
			() => this.instance.Size.add(offset),
			params,
		) as unknown as OldTransformBuilder<T>;
	}

	slideIn(this: OldTransformBuilder<GuiObject>, from: Direction, power: number, props?: TransformProps) {
	...
	slideOut(this: OldTransformBuilder<GuiObject>, direction: Direction, power: number, props?: TransformProps) {
		return this.moveRelative(new UDim2().sub(directionToOffset(direction, power)), {
			...
			.move(this.instance.Position) as unknown as OldTransformBuilder<T>;
	}
	...
	flashColor<TKey extends ExtractKeys<T & GuiObject, Color3>>(
		...
		return this.flash(color as never, property as never, props) as unknown as OldTransformBuilder<T>;
	}
}

//

export class OldTransformRunner extends Component implements RunningTransform {
